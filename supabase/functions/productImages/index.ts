...
app.use(async (ctx) => {
  ...
  const url = // manually construct the URL, since getPublicUrl is not working as expected
    Deno.env.get("SUPABASE_URL") +
    "/storage/v1/object/public/" +
    upload.fullPath

  const productId = formData.fields!.id || ""
  ...
  if (deleteError) {
    ...
    ctx.response.status = 500
    ctx.response.body = "Fail to delete the record"
    return
  }
  // insert record to product images table
  const { error } = await supabaseClient.from("productImages").insert({
    productId,
    path: upload.path,
